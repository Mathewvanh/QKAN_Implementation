...
# Make extend_grid a standalone function
def extend_grid(grid, k_extend=0):
    """
    Pad k grid points to left and right
    
    Args:
    -----
        grid: 2D torch.tensor
            Grid points, shape (number of splines, number of grid points)
        k_extend: int
            Number of points to extend on each side
            
    Returns:
    --------
        extended grid: 2D torch.tensor
            Extended grid with k_extend points added to each side
    """
    # grid shape: (batch, grid)
    if k_extend <= 0:
        return grid
    else:
        # Match the original implementation: equal spacing based on grid step
...
def B_batch(x, grid, k=0, extend=True, device='cpu'):
    ...
    -----
        ...
        device : str
            device (Note: this is kept for backward compatibility but will be ignored in favor of using x's device)
    ...
    '''
    # Use the device from input tensor
    actual_device = x.device
    
    if extend:
        grid = extend_grid(grid, k_extend=k)
    
    # base case: k = 0, box function
    if k == 0:
        # Make sure both x and grid are 2D
        if x.dim() > 2:
        ...
        # Handle case where grid has an extra dimension (3D) during recursion
        if grid.dim() > 2:
            # This is likely happening during recursion, need to reshape
            if grid.dim() == 3:
                # For recursive calls, reshape correctly
                grid_for_recursion = grid[:, 0, :]
                B_km1 = B_batch(x, grid=grid_for_recursion, k=k-1, extend=False, device=actual_device)
                # Compute remaining values
                if grid.shape[1] >= (k+1) and grid.shape[2] >= (k+1):
                    value = ((x - grid[:, 0, :-(k+1)]) / (grid[:, 0, k:-1] - grid[:, 0, :-(k+1)])) * B_km1[:, :-1] + \
                           ((grid[:, 0, k+1:] - x) / (grid[:, 0, k+1:] - grid[:, 0, 1:(-k)])) * B_km1[:, 1:]
                else:
                    # If grid doesn't have enough elements for computation, return zeros
                    value = torch.zeros_like(B_km1)
            else:
                # Handle other unexpected dimensionalities
                value = torch.zeros((x.shape[0], grid.shape[1]-k, x.shape[1]), device=actual_device)
        else:
            # Normal case - 2D grid
            x_for_recursion = x if x.dim() == 1 else x[:, 0]
            B_km1 = B_batch(x_for_recursion, grid=grid, k=k-1, extend=False, device=actual_device)
            
            # Compute the terms if dimensions allow
            if grid.shape[1] >= (k+1):
                value = ((x - grid[:, :-(k+1)]) / (grid[:, k:-1] - grid[:, :-(k+1)])) * B_km1[:, :-1] + \
                       ((grid[:, k+1:] - x) / (grid[:, k+1:] - grid[:, 1:(-k)])) * B_km1[:, 1:]
            else:
                # If grid doesn't have enough elements, return zeros
                value = torch.zeros_like(B_km1)
...
def coef2curve(x_eval, grid, coef, k, device="cpu"):
    ...
    -----
        ...
        k : int
            the piecewise polynomial order of splines.
        device : str
            device (Note: this is kept for backward compatibility but will be ignored in favor of using x_eval's device)
    ...
    --------
        y_eval : 2D torch.tensor
            shape (number of splines, number of samples)
    '''
    # Use the device from input tensor rather than the parameter
    actual_device = x_eval.device
    ...
    grid_ext = extend_grid(grid, k_extend=k)
    
    # Compute B-spline values
    B_values = B_batch(x_eval, grid_ext, k=k, device=actual_device)
    
    # Ensure coef is 2D
    if coef.dim() > 2:
        coef = coef.reshape(coef.shape[0], -1)
    
    # Compute curve values by applying coefficients to B-spline basis values
    y_eval = torch.sum(coef.reshape(coef.shape[0], -1, 1) * B_values, dim=1)
...
def curve2coef(x_eval, y_eval, grid, k, device="cpu"):
    ...
    --------
        coef : 2D torch.float
            shape (number of splines, number of coef params)
    '''
    # Use the device from input tensor
    actual_device = x_eval.device
    ...
    B_transpose = B_values.transpose(1, 2)  # (size, batch, basis)
    B_dot = torch.bmm(B_transpose, B_values)  # (size, basis, basis)
    B_dot_inv = torch.linalg.pinv(B_dot)  # (size, basis, basis)
    By = torch.bmm(B_transpose, y_eval.unsqueeze(2))  # (size, basis, 1)
    coef = torch.bmm(B_dot_inv, By).squeeze(2)  # (size, basis)
...
class KANLayer(nn.Module):
    ...
    def forward(self, x):
        '''
        Forward pass to compute KAN layer output from input
        
        Args:
        -----
            x : 2D torch.float
                batched input data, shape (batch, in_dim)
        
        Returns:
        --------
            y : 2D torch.float 
                outputs of the forward pass, shape (batch, out_dim)
            preacts : 3D torch.float
                the raw input values (aka preactivation values) for each output dimension, shape (batch, out_dim, in_dim)
            postacts : 3D torch.float
                the preactivations after multiplying with learned weights, shape (batch, out_dim, in_dim)
        ...
        '''
        batch = x.shape[0]
        # Get the current device from the input tensor
        current_device = x.device
        
        # x: shape (batch, in_dim) => shape (size, batch) (size = out_dim * in_dim)
        x = torch.einsum('ij,k->ikj', x, torch.ones(self.out_dim, device=current_device)).reshape(batch,
        ...
        y = coef2curve(x_eval=x, grid=self.grid[self.weight_sharing], coef=self.coef[self.weight_sharing], k=self.k,
                       device=current_device)  # shape (size, batch)
